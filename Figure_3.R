...
# Figure 3
    ...
    # Log Likelihood
    boxplot(-lik_out_resp[1,],-lik_out_resp[2,],-lik_out_resp[3,],-lik_out_resp[4,],-lik_out_resp[5,],outline=F,xlab="",ylab="-Log likelihood",ylim=c(-150,-116),cex.lab=1.2,frame=F)
    axis(1,at=c(-1,1,2,3,4,5,10),labels=c("","1","2","3","4","5",""))
    axis(2,at=c(-1000,1000),c("",""))
    text(c(1:5),rep(-115.5,5),c("a","b","c","d","d"))
    mtext("Scenario",1,cex=1.2,line=2.25)
    mtext("A: Function",3,line=1,at=3,cex=1.45)
    
    # Bray-Curtis similarity
    boxplot(-BC_sim[1,]/(4*29),-BC_sim[2,]/(4*29),-BC_sim[3,]/(4*29),-BC_sim[4,]/(4*29),-BC_sim[5,]/(4*29),outline=F,xlab="",ylab="- Bray-Curtis index",ylim=c(-1,-0.58),cex.lab=1.2,frame=F)
    axis(1,at=c(-1,1,2,3,4,5,10),labels=c("","1","2","3","4","5",""))
    axis(2,at=c(-1000,1000),c("",""))
    text(c(1:5),rep(-0.578,5),c("a","b","c","d","e"))
    mtext("B: Composition",3,line=1,at=3,cex=1.45)
    mtext("Scenario",1,cex=1.2,line=2.25)
